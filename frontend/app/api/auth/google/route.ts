...
export const GET = async (req: NextRequest) => {
  ...
  const tolkenBaseUrl = "https://oauth2.googleapis.com/token";
  const config = {
    code,
    client_id: clientId,
  ...
  const response = await fetch(tolkenBaseUrl, {
    method: "POST",
  ...
  const userUrl = "https://www.googleapis.com";
  const userData = await (
    await fetch(`${userUrl}/userinfo/v2/me`, {
      headers: {
